...
export class Basket extends View<IBasket> {
	...
	constructor(container: HTMLElement, protected events: IEvents) {
		...
		this._itemList = this.ensure<HTMLElement>(settings.basketListSelector);
		this._totalPrice = this.ensure<HTMLElement>(settings.basketPriceSelector);
		this._checkoutButton = this.ensure<HTMLButtonElement>(
			settings.basketButtonSelector
		);

		this._checkoutButton.addEventListener(
			'click',
