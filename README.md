...
Абстрактный класс ```ModalView``` базовый класс для модальных окон, расширяет абстрактный класс ```View```
...
onClick(): void - вызывает отображение модального окна корзины
...
products: ProductData[] - массив элементов

Конструктор тот же что и у базового класса

Методы: 

render(): void - пробегает по списку продуктов и для каждого вызывает рендер, также навешивает на весь список слушателя событий 'click'

renderProduct(): void - отображает карточку товара

### ProductModalView

Представление для отображения модального окна товара наследуется от абстрактного класса ```ModalView```
...
render(): void - рендерит внутри модального окна данные продукта и вызывает метод show(), навешивает событие click на кнопку "Купить"
...
render(): void - пробегает по списку продуктов и для каждого вызывает рендер

remove(): void - удаляет элемент со страницы и из корзины

### BasketModalView

Представление для отображения модального окна корзины наследуется от абстрактного класса ```ModalView```

Поля класса:

items: Pick<ProductData, 'id' | 'price' | 'title'>[] - список элементов корзины
...
render(): void - рендерит внутри модального окна данные о продуктах, добавленных в корзину и вызывает метод show(), навешивает событие click на кнопку "Оформить",

а также на список товаров, для отслеживания удаления элемента из корзины

onRemove(id: string) - метод, который вызывается при удалении товара из корзины

set total(total: number) - метод, который вызывается при удалении товара

set items(items: Pick<ProductData, 'id' | 'price' | 'title'>[]) - обновляет список товаров в корзине

### OrderFormView

Представление для отображения модального окна оплаты наследуется от абстрактного класса ```ModalView```

Поля класса:

address: string
selectedPayment: 'online' | 'offline' | null
...
set selectedPayment('online' | 'offline' | null) 

validateForm(): void валидирует форму после изменения ее полей

isValid(): boolean проверяет поля на валидность

### ContactsFormView

Представление для отображения модального окна ввода данных наследуется от абстрактного класса ```ModalView```
...
set email('online' | 'offline' | null) 

validateForm(): void валидирует форму после изменения ее полей

isValid(): boolean проверяет поля на валидность

validateEmail(): boolean проверяет валиден ли email

validatePhone(): boolean проверяет валиден ли введенный телефонный номер

### SuccessModalView

Представление для отображения модального окна успешного заказа наследуется от абстрактного класса ```ModalView```
